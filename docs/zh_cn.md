...
`pos`: *Position*,   指定要加载的区块。应该提供该区块内的任意方块坐标。可以为`{x:<x>,z:<z>}`的形式，也可以为一个长度为2或3的列表表示`[x, z]`或`[x, y, z]`(y若提供会被忽略)。 若不指定`pos`则会自动使用当前的执行位置。\
`to`: *Position*,  与`pos`相同。若存在，选定两角之间长方形范围内的所有区块。\
`dimension`: *#[id="dimension"] string,*   该请求的维度。若不提供则使用当前上下文。\
